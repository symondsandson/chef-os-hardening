...
sysctl_param 'net.ipv4.ip_forward' do
  value lazy { node['os-hardening']['network']['forwarding'] ? 1 : 0 }
end
sysctl_param 'net.ipv6.conf.all.forwarding' do
  value lazy { node['os-hardening']['network']['ipv6']['enable'] && node['os-hardening']['network']['forwarding'] ? 1 : 0 }
end

# Disable or Enable IPv6 as it is needed.
sysctl_param 'net.ipv6.conf.all.disable_ipv6' do
  value  lazy { node['os-hardening']['network']['ipv6']['enable'] ? 0 : 1 }
end
...
sysctl_param 'net.ipv4.conf.all.arp_ignore' do
  value lazy { node['os-hardening']['network']['arp']['restricted'] ? 1 : 0 }
end
...
sysctl_param 'net.ipv4.conf.all.arp_announce' do
  value lazy { node['os-hardening']['network']['arp']['restricted'] ? 2 : 0 }
end
...
sysctl_param 'kernel.sysrq' do
  value lazy { node['os-hardening']['security']['kernel']['enable_sysrq'] ? node['os-hardening']['security']['kernel']['secure_sysrq'] : 0 }
end

# Prevent core dumps with SUID. These are usually only needed by developers and
# may contain sensitive information.
sysctl_param 'fs.suid_dumpable' do
  value lazy { node['os-hardening']['security']['kernel']['enable_core_dump'] ? 2 : 0 }
end
